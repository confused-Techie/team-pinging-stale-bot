...
module.exports = class Stale {
  constructor (github, { owner, repo, logger = console, ...config }) {
    ...
    this.org = owner;
    this.teams = this.github.rest.teams.list({ this.org }); // we only care about this.teams[i].slug
  ...
  async findTeamForRepo(repo) {
    for (let i = 0; i < this.teams.length; i++) {
      let teamsRepos = this.github.rest.teams.listReposInOrg({ this.org, this.teams[i].slug });
